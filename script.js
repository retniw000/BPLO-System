const API = 'api.php';
    ...
    const FULL_COLS = [
      ...
      // Application — Demographics & Assets
      ...
      { key: 'Corporate_Name',              label: 'Corporate Name',      group: 'tax',  cls: 'g-tax'  },
    ...
    // Single tab — Application (Full Joined View, CRUD-enabled)
    const TABS = [
      {
        key:        'fullView',
        label:      'Application',
    ...
    /* --------------------------------------------------
       FORM GROUP DEFINITIONS (for Add / Edit / View)
    -------------------------------------------------- */
    ...
    const ADD_GROUPS = [
      ...
      },
      {
        ...
        icon:   'fa-building',
        fields: [
          'TIN', 'Corporate_Name', 'Taxpayer_FullName', 'Trade_Name',
          'Business_Organization_Type', 'Owner_Gender',
          'Office_Classification', 'Office_Address',
        ],
      },
      {
        ...
        icon:   'fa-user-tie',
        fields: [
          'Auth_Rep_FullName', 'Auth_Rep_Position',
          'Auth_Rep_Telephone', 'Auth_Rep_MobileNumber', 'Auth_Rep_Email',
        ],
      },
      {
        title:  '4. Safety Officer',
        icon:   'fa-user-shield',
        fields: [
          'Safety_Officer_FullName', 'Safety_Officer_Telephone',
          'Safety_Officer_MobileNumber', 'Safety_Officer_Email',
        ],
      },
      {
        ...
        fields: [
          'Emp_Estab_Male', 'Emp_Estab_Female',
          'Emp_Taguig_Male', 'Emp_Taguig_Female',
          'Truck_Vehicle_Count', 'Van_Vehicle_Count',
          'Motorcycle_Vehicle_Count', 'Other_Vehicle_Count',
        ],
      },
    ];

    // Primary keys locked during edit
    const EDIT_PKS = new Set(['LCN', 'TIN', 'Auth_Rep_ID', 'Safety_Officer_ID', 'PSIC']);

    // Field metadata derived from column definitions
    const FIELD_META = {
    ...
    // UI state (kept in-memory on the page):
    // - activeTab: which tab is being shown
    // - allRows: joined table rows used for rendering/filtering
    // - selectedRow: the currently highlighted record
    // - formMode: add/edit/view mode for the modal


    let activeTab   = 0;
    let allRows     = [];
    let selectedRow = null;
    let formMode    = 'add'; // 'add', 'edit', 'view'
    ...
    async function loadFullView() {
      ...
      // Join: one row per LCN + PSIC combination
      const joined = [];
      (apps || []).forEach(app => {
    ...
    function renderTable(rows, filtered = false) {
      ...
      if (!rows.length) {
        ...
        rows.forEach((row, i) => {
          const isSelected = selectedRow
            && selectedRow.LCN  === row.LCN
            && selectedRow.PSIC === row.PSIC;
      ...
      document.getElementById('tabHint').textContent =
        `Application — ${FULL_COLS.length} columns · 1 row per application × LOB`;
    }
    ...
    function openEdit() {
      ...
      // Pre-fill the single LOB row from the selected record
      addLobRow({
    ...
    function openView() {
      ...
      // Pre-fill ALL LOB rows for the selected application (same LCN)
    ...
    async function submitAdd() {
      ...
      try {
        ...
        loadTable();
      } catch { /* network error already toasted */ }
      finally {
        btn.disabled    = false;
    ...
    function openDelete() {
      ...
      document.getElementById('delTitle').textContent =
        `Delete Application — ${esc(selectedRow.LCN || '')}`;

      document.getElementById('delDesc').textContent =
        'One request removes all related data in a single transaction.';
    ...
    // Server API wrapper (standardizes fetch + error handling so UI code can stay clean).
    ...
    // App entry point: build the tab bar and load the initial table.
